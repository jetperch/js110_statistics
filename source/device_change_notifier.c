...
static js110_device_change_notifier_callback callback_ = NULL;
...
static const LPCWSTR szMainWndClassW = L"DeviceChangeWindow";
static const LPCSTR szMainWndClass = "DeviceChangeWindow";
...
static LRESULT CALLBACK window_callback(HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam) {
    switch (nMsg) {
        case WM_CLOSE: {
            DEBUG_PRINTS("WM_CLOSE");
            DestroyWindow(hwnd);
            return DefWindowProc(hwnd,  nMsg, wParam, lParam);
        }

        case WM_DESTROY: {
            DEBUG_PRINTS("WM_DESTROY");
        ...
        case WM_DEVICECHANGE: {
            ...
            DEBUG_PRINTS("WM_DEVICECHANGE (filtered)");
...
static DWORD WINAPI window_create(LPVOID lpParam) {
    ...
    if (!RegisterClassEx(wndclass)) {
        DEBUG_PRINTS("RegisterClassEx failed");
        return 2;
    }

    window_ = CreateWindowW(
        szMainWndClassW,            /* Class name */
        szMainWndClassW,            /* Caption */
    ...
    if (window_ == NULL) { // we have a problem!
        DEBUG_PRINTS("CreateWindowW failed");
        return 3;
    }
    ...
    if (NULL == windowEvent_) {
        DEBUG_PRINTS("RegisterDeviceNotification -> windowEvent_ is null");
        rc = 4;
    } else {
    ...
    if (!UnregisterClass(wndclass->lpszClassName, wndclass->hInstance)) {
        DEBUG_PRINTS("Could not unregister window");
    } else {
        free(wndclass);
    }
...
int js110_device_change_notifier_initialize(js110_device_change_notifier_callback callback, void * cookie) {
...
int js110_device_change_notifier_finalize(void) {
    int rc = 0;
